...
export function XCaptureProbe({ initialPostId }: XCaptureProbeProps): ReactElement {
  ...
  async function onSubmit(event: FormEvent): Promise<void> {
    ...
    try {
      ...
      const response = captureProbeResponseSchema.safeParse(await browser.runtime.sendMessage({
        type: 'x-capture:probe', tabId: tab.id, postId: parsed.data,
      }))
    ...
    <section className="border-t border-border p-3" aria-label="X capture diagnostics">
      <form onSubmit={onSubmit} className="flex flex-col gap-2">
        ...
        <p className="text-xs text-text-muted">
          Refresh the permalink first. Reads and discards media bytes; nothing is saved to your graph.
          Keep this popup open while reading.
        </p>
        <label htmlFor="x-probe-post-id" className="text-xs text-text-secondary">Post ID</label>
        <input id="x-probe-post-id" value={postId} onChange={(event) => setPostId(event.target.value)}
          inputMode="numeric" disabled={pending}
      ...
      {state.phase === 'complete' ? (
        <div className="mt-3 flex flex-col gap-2 text-xs" aria-live="polite">
          ...
          <p>Videos/GIFs: {state.report.counts.videoCount}; MP4 available: {state.report.counts.mp4Available};
            HLS-only: {state.report.counts.hlsOnly}; no source: {state.report.counts.noSource}.</p>
          {([['Post', state.report.post], ['Quote', state.report.post.quote]] as const).map(([label, post]) => (
            post ? (
              <div key={label}>
                <p className="font-medium">{label}: @{post.author.handle}</p>
                {post.truncated ? <p className="text-destructive">Truncated: expand the post and retry.</p> : null}
                <p className="max-h-48 overflow-auto whitespace-pre-wrap break-words">{post.body.map((segment) => segment.text).join('') || '(No text)'}</p>
              </div>
            ) : null
          ))}
          <ul className="flex flex-col gap-1">
            {state.report.results.map((result) => (
              <li key={result.slot} className="break-words">
                {result.slot}: {result.status === 'read'
          ...
          <details>
            <summary className="cursor-pointer text-text-muted">Byte signatures and document token</summary>
            <pre className="max-h-48 overflow-auto whitespace-pre-wrap break-all">{JSON.stringify({
              documentToken: state.report.documentToken, results: state.report.results,
            }, null, 2)}</pre>
          </details>
        </div>
      ) : null}
