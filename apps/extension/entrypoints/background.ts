...
export default defineBackground(() => {
  ...
  browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (import.meta.env.DEV && sender.id === browser.runtime.id && sender.tab == null &&
      ...
      if (request.success) {
        void import('@/lib/x-capture-probe').then(async ({ runCaptureProbe }) => {
          const report = await runCaptureProbe(request.data.tabId, request.data.postId)
          sendResponse({ ok: true, report })
        }).catch((error: unknown) => sendResponse({
          ok: false, reason: error instanceof Error ? error.message : 'probe-failed',
        }))
        return true
      }
    }
