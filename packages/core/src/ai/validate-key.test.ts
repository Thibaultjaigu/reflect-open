...
describe('validateApiKey', () => {
  it('sends the provider-specific auth headers to the model-listing endpoint', async () => {
    ...
    await validateApiKey({ provider: 'requesty', apiKey: 'rqsty-test' }, recordingFetch)
    ...
    expect(calls[4]!.url).toBe('https://router.requesty.ai/v1/models')
    expect(calls[4]!.headers['Authorization']).toBe('Bearer rqsty-test')
  ...
  it('reads an auth rejection as invalid', async () => {
    ...
    expect(
      await validateApiKey({ provider: 'requesty', apiKey: 'rqsty-test' }, fetchReturning(403)),
    ).toBe('invalid')
    expect(
      await validateApiKey(
        { provider: 'openai-compatible', apiKey: '', baseUrl: 'http://localhost:1234/v1' },
