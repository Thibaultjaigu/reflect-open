...
describe('media selection', () => {
  it('keeps both posts and posters, selects one MP4, and preserves unavailable diagnostics', () => {
    ...
    expect(collectProbeMedia(post)).toEqual([
      { slot: 'post.avatar', unavailable: false, kind: 'image', url: 'https://pbs.twimg.com/avatar.jpg' },
      { slot: 'post.media.0', unavailable: false, kind: 'image', url: 'https://pbs.twimg.com/photo.jpg' },
      { slot: 'post.media.1.poster', unavailable: false, kind: 'image', url: 'https://pbs.twimg.com/poster.jpg' },
      { slot: 'post.media.1', unavailable: false, kind: 'video', url: 'https://video.twimg.com/high.mp4' },
      { slot: 'quote.media.0.poster', unavailable: true, kind: 'image', url: 'https://pbs.twimg.com/quote.jpg' },
  ...
  it('does not confuse missing sources with HLS-only and supports MP4 GIFs without bitrate', () => {
    ...
    post.media = [
      ...
      { type: 'gif', width: 1, height: 1, sources: [
        { type: 'video/mp4', url: 'https://video.twimg.com/gif.mp4' },
      ] },
    ]
    expect(collectProbeMedia(post).slice(1)).toEqual([
      { slot: 'post.media.0', unavailable: false, kind: 'unsupported', reason: 'no-source' },
      { slot: 'post.media.1', unavailable: false, kind: 'video', url: 'https://video.twimg.com/gif.mp4' },
    ])
  })
})
...
describe('media reader', () => {
  it.each([
    'http://pbs.twimg.com/photo.jpg', 'https://pbs.twimg.com.evil.test/photo.jpg',
    'https://pbs.twimg.com@evil.test/photo.jpg', 'https://user:password@pbs.twimg.com/photo.jpg',
    'https://pbs.twimg.com:8443/photo.jpg', 'http://localhost/file', 'not-a-url',
  ...
  it('reads every chunk, retains only 32 signature bytes, and uses credentialed non-redirecting fetch', async () => {
    ...
    await expect(probeMedia('https://video.twimg.com/movie.mp4', 'video')).resolves.toEqual({
      bytes: 42, mime: 'video/mp4', signature: [1, 2, ...new Array<number>(30).fill(3)],
    })
  ...
  it('reports an empty body instead of a successful read', async () => {
    ...
    await expect(probeMedia('https://pbs.twimg.com/photo.png', 'image')).rejects.toThrow('empty-media-body')
  })
  ...
  it('cancels an oversized stream and releases its lock without retaining the full media', async () => {
    ...
    await expect(probeMedia('https://video.twimg.com/large.mp4', 'video')).rejects.toThrow('probe-size-budget')
    expect(cancel).toHaveBeenCalledOnce()
    expect(stream.locked).toBe(false)
  })
  ...
  it('distinguishes a deadline from network errors and does not expose signed URLs in errors', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn().mockRejectedValue(new Error('https://video.twimg.com/secret?token=private')))
    try {
      await expect(probeMedia('https://video.twimg.com/movie.mp4', 'video')).rejects.toThrow('probe-time-budget')
    } finally {
      timeout.mockRestore()
    }
    await expect(probeMedia('https://video.twimg.com/movie.mp4', 'video')).rejects.toThrow('media-network-error')
  })
})
